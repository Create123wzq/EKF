...
class TrackerEKF(EKF):
    ...
    def __init__(self, M, s0, q0, points0, w0, v0, p, q, r):
        EKF.__init__(self, 3*M+12, 2*M, pval=p, qval=q, rval=r)
        #初始状态估计
...
if __name__ == '__main__':
    number = 50
    prefix = "/Users/apple/Desktop/test/img5/"
    sift_points, number = getPositions(prefix, number, ratio=0.7)
    sift_points = list(sift_points.values())
    M = len(sift_points) # 卡尔曼滤波器跟踪的点的个数
    M = 4
    # 测量值列表
    measured_points = []
    quaternions = []
    ...
    # 估计值列表
    kalman_points = []
    # 初值:小行星坐标系原点在相机坐标系下的位置(y值要取反！！！！！！！！！！)
    s0 = [0., 0., 8484]
    # 初值:小行星坐标系到相机坐标系坐标系旋转矩阵(四元数)
    q0 = [-0.383, 0., 0., 0.924]

    # M个特征点在小行星坐标系下的坐标（不随时间变化,y值要取反！！！！！！！！！！)
    points = [[-244., -157., -756.], [-332., -372., -369.], [251, -255, -117], [326, -278, -56]]
    # 初值：小行星坐标系x,y,z方向的角速度初值
    w0 = [0., 0 , 0.]
    # 初值：小行星坐标原点在相机坐标系下x,y,z方向速度初值
    v0 = [0., 0., 0.]

    kalfilt = TrackerEKF(M, s0, q0, points, w0, v0, p=0.02, q=0.0002, r=0.000002)
